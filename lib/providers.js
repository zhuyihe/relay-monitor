...
/**
 * 按日志里的 token_id 精确汇总某把固定 Key 的窗口消费。
 * /api/log/self/stat 只能按 token_name 汇总，删除并复用同名 Key 时会串账，不能作为利润口径。
 */
export async function queryNewApiTokenWindow(station, { tokenId, tokenName, startMs, endMs }) {
  const wantedTokenId = Number(tokenId);
  if (!Number.isFinite(wantedTokenId) || wantedTokenId <= 0) throw new Error("上游 Key ID 无效");
  const { base, headers } = newApiStationAuth(station);
  const window = unixSecondWindow(startMs, endMs);
  const PAGE = 100; // new-api 的 page_size 上限
  const MAX_ROWS = 10000; // 上游用户日志常见的可检索上限；碰到上限时必须拒绝不完整金额
  const baseQs = {
    type: "2",
  ...
  const unavailable = (message, code = "UPSTREAM_LOG_UNAVAILABLE") => {
    const err = new Error(message);
    err.code = code;
    return err;
  };
  const fetchPage = async (page) => {
    const qs = new URLSearchParams({ ...baseQs, p: String(page), page_size: String(PAGE) });
    const r = await request(`${base}/api/log/self?${qs}`, { headers, timeoutMs: 25000 });
    if (r.status >= 300 || r.body?.success === false) {
      const error = newApiResponseError(r, "读取上游消费日志失败");
      throw unavailable(error.message, error.code);
    }
    const total = Number(r.body?.data?.total);
    return { items: newApiItems(r.body), total: Number.isFinite(total) ? total : null };
  };

  const [status, first] = await Promise.all([newApiStatus(base), fetchPage(1)]);
  // 10,000 是当前 NewAPI 用户日志查询的保护上限。总数碰到它时不能确认是否被上游截断。
  if (first.total != null && first.total >= MAX_ROWS) {
    throw unavailable("上游日志达到可安全读取上限，无法完成精确对账", "UPSTREAM_LOG_LIMIT");
  }
  const rows = [...first.items];
  if (first.total == null) {
    for (let page = 2; page <= MAX_ROWS / PAGE && rows.length < MAX_ROWS; page++) {
    ...
    if (rows.length >= MAX_ROWS) {
      throw unavailable("上游未提供日志总数且已达到扫描上限，无法完成精确对账", "UPSTREAM_LOG_LIMIT");
    }
  } else {
    const pages = Math.ceil(Math.max(first.total, PAGE) / PAGE);
    for (let page = 2; page <= pages && rows.length < first.total; page += 4) {
      const batch = [];
      for (let offset = 0; offset < 4 && page + offset <= pages; offset++) batch.push(fetchPage(page + offset));
      const result = await Promise.all(batch);
      if (result.some((entry) => !entry.items.length)) {
        throw unavailable("上游日志分页不完整，已停止精确对账");
      }
      for (const entry of result) rows.push(...entry.items);
    }
  ...
  for (const row of rows) {
    ...
    const rowTokenId = Number(row?.token_id);
    if (!Number.isFinite(logId) || logId <= 0 || !Number.isFinite(rowTokenId) || rowTokenId <= 0) {
      throw unavailable("上游日志未提供 token_id，当前版本无法安全精确对账", "UPSTREAM_TOKEN_ID_UNAVAILABLE");
    }
    if (rowTokenId !== wantedTokenId) continue;
    facts.push({
      id: logId,
      tokenId: rowTokenId,
      quotaUnits: num(row?.quota),
      group: String(row?.group || ""),
      createdAtMs: num(row?.created_at) * 1000,
    });
  }
  return {
    ...status,
    quotaUnits: facts.reduce((sum, fact) => sum + fact.quotaUnits, 0),
    actualGroups: [...new Set(facts.map((fact) => fact.group).filter(Boolean))],
    tokenIds: [...new Set(facts.map((fact) => fact.tokenId))],
    latestLogAtMs: facts.reduce((latest, fact) => Math.max(latest, fact.createdAtMs), 0) || null,
    scanned: rows.length,
    total: first.total,
    facts,
  };
...
/**
 * 本站管理端渠道收费汇总。保留原始 quota，用 status 的单位转换金额；
 * flow 与模型总额同时读取，用于向 UI 暴露覆盖率而不是把缺失计为零。
 */
export async function queryOwnChannelRevenue(station, { channelIds, startMs, endMs }) {
  ...
  const { base } = newApiStationAuth(station);
  let status, flowRows, modelRows;
  try {
    [status, flowRows, modelRows] = await Promise.all([
      newApiStatus(base),
      queryOwnFlow(station, startMs, endMs, { includeQuotaUnits: true }),
      queryOwnData(station, startMs, endMs, "model", { includeQuotaUnits: true }),
    ]);
  } catch (cause) {
    const err = new Error("无法读取本站渠道收费数据");
    err.code = cause?.code || "OWN_FLOW_UNAVAILABLE";
    throw err;
  }
  const byChannel = new Map();
  let flowQuotaUnits = 0;
  for (const row of flowRows) {
    const units = num(row.quotaUnits);
    flowQuotaUnits += units;
    if (!wanted.has(row.channelId)) continue;
    const current = byChannel.get(row.channelId) || {
      channelId: row.channelId,
      name: row.channelName || `渠道 ${row.channelId}`,
      quotaUnits: 0,
    };
    current.quotaUnits += units;
    byChannel.set(row.channelId, current);
  }
  const modelQuotaUnits = modelRows.reduce((sum, row) => sum + num(row.quotaUnits), 0);
  const selected = [...byChannel.values()];
  const quotaUnits = selected.reduce((sum, row) => sum + row.quotaUnits, 0);
  const coverage = modelQuotaUnits > 0 ? flowQuotaUnits / modelQuotaUnits : 1;
  return {
    ...status,
    quotaUnits,
    coverage,
    channels: selected.map((row) => ({
      ...row,

...
/**
 * 按固定 Key 名称与分组读取上游窗口统计。调用方必须先用 token ID 元数据确认 Key
 * 仍是创建规则时的那一把，避免同名 Key 被替换后混入聚合结果。
 */
export async function queryNewApiTokenStat(station, { tokenName, group, startMs, endMs }) {
  const name = String(tokenName || "").trim();
  const fixedGroup = String(group || "").trim();
  if (!name || !fixedGroup) throw new Error("上游 Key 名称或固定分组无效");
  const { base, headers } = await newApiReconciliationAuth(station);
  const window = unixSecondWindow(startMs, endMs);

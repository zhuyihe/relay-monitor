...
import { queryAdminTokens, queryNewApiReconciliationMetadata, queryOwnChannels, queryStation } from "./providers.js";
...
test("本站渠道超过 500 条时仍能拉到最后一页的新渠道", async (t) => {
  ...
  assert.deepEqual(channels.at(-1), { id: 501, name: "小福星-awsb_3-2.9", type: 0, status: 1, baseUrl: "" });
...
test("对账分段复用已读到的 PAT 身份与 /api/status，复用值无效时照样报错", async (t) => {
  ...
  await assert.rejects(
    () => queryOwnChannelRevenue(station, { channelIds: [1], ...window, loadStatus: async () => ({ version: "own" }) }),
    (error) => error.code === "OWN_BILLING_UNAVAILABLE" && /quota_per_unit/.test(error.cause?.message)
  );

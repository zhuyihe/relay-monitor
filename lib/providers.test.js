...
test("渠道对账使用固定 Key 名称和分组的统计接口", async (t) => {
  let selfCalls = 0;
  ...
  const server = createServer((request, response) => {
    ...
    if (url.pathname === "/api/log/self/stat") {
      ...
      assert.equal(url.searchParams.get("group"), "oai");
      assert.equal(url.searchParams.get("start_timestamp"), "1000");
  ...
  const upstream = await queryNewApiTokenStat(station, { tokenName: "oai", group: "oai", startMs: 1000000, endMs: 1060000 });
  ...
  assert.equal(selfCalls, 2);
  await queryNewApiTokenStat(configuredStation, { tokenName: "oai", group: "oai", startMs: 1000000, endMs: 1060000 });
  assert.equal(selfCalls, 2);
...
test("上游统计使用已有 Bearer PAT 且认证失败不会泄露凭证", async (t) => {
  ...
  await assert.rejects(
    queryNewApiTokenStat({ baseUrl: origin, accessToken: "Bearer already-prefixed" }, {
      tokenName: "fixed-key", group: "fixed", startMs: 1000000, endMs: 1060000,
    }),

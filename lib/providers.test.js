...
test("渠道对账按日志 token_id 精确汇总，排除历史同名 Key", async (t) => {
  const server = createServer((request, response) => {
    const url = new URL(request.url, "http://x");
    if (url.pathname === "/api/status") {
      return sendJson(response, { success: true, data: { version: "test", quota_per_unit: 100 } });
    }
    assert.equal(request.headers.authorization, "pat");
    ...
    if (url.pathname === "/api/log/self") {
      ...
      const items = page === 1
        ? [
          { id: 1, token_id: 9, token_name: "oai", group: "oai", created_at: 1040, quota: 80 },
          ...Array.from({ length: 99 }, (_, index) => ({ id: index + 2, token_id: 7, token_name: "oai", group: "oai", created_at: 1030, quota: 999 })),
        ]
        : [{ id: 101, token_id: 9, token_name: "oai", group: "oai", created_at: 1050, quota: 320 }];
      return sendJson(response, { success: true, data: { total: 101, items } });
    }
  ...
  t.after(() => server.close());
  const origin = `http://127.0.0.1:${server.address().port}`;
  const { queryNewApiReconciliationMetadata, queryNewApiTokenWindow, queryOwnChannelRevenue } = await import("./providers.js");
  ...
  const upstream = await queryNewApiTokenWindow(station, { tokenId: 9, tokenName: "oai", startMs: 1000000, endMs: 1060000 });
  assert.equal(upstream.quotaUnits, 400);
  assert.equal(upstream.quotaPerUnit, 100);
  assert.deepEqual(upstream.tokenIds, [9]);
  assert.equal(upstream.scanned, 101);
